...
export class FollowService {
    ...
    private resourceUrlDate =  SERVER_API_URL + 'api/follows/next-contact';
    private resourceUrlRegistryDate =  SERVER_API_URL + 'api/follows/regsitry-date';
    ...
    findBydate(date: any): Observable<HttpResponse<Follow[]>> {
        return this.http.get<Follow[]>(`${this.resourceUrlDate}/${date}`, { observe: 'response'})
            .map((res: HttpResponse<Follow[]>) => this.convertArrayResponse(res));
    }

    findByRegistryDate(date: any): Observable<HttpResponse<Follow[]>> {
        return this.http.get<Follow[]>(`${this.resourceUrlRegistryDate}/${date}`, { observe: 'response'})
            .map((res: HttpResponse<Follow[]>) => this.convertArrayResponse(res));
    }
