...
public class ContactResource {
    ...
    @PostMapping("/contacts/find-email")
    @Timed
    public ResponseEntity<ContactDTO> findByEmail(@RequestBody ContactDTO contactDTO) throws URISyntaxException {

        ContactDTO result = contactService.findByEmail(contactDTO.getEmail());
        return ResponseEntity.created(new URI("/api/contacts/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(ENTITY_NAME, result.getId().toString()))
            .body(result);
    }
